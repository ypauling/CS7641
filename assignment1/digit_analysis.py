...
solver_knn5 = KNeighborsClassifier(n_neighbors=5)
plot_sklearn_learning_curve(solver_knn5, X, y, 3,
                            'K Nearest Neighbor (K = 5)', 'knn5_sklearn_learning_curve.png')
...
tree_pre_pruning = np.zeros((5, 20, 6), dtype=np.float32)
m, n, s = (0, 0, 0)
for i, m in enumerate(np.linspace(1, 15, 5, dtype=np.int16)):
    for j, n in enumerate(np.linspace(2, 166, 20, dtype=np.int16)):
        for k, s in enumerate(np.linspace(1, 12, 6, dtype=np.int16)):
            solver_tree = DecisionTreeClassifier(
                max_depth=m, max_leaf_nodes=n, min_samples_leaf=s)
            tree_pre_pruning[i, j, k] = np.mean(
...
# There is only one best tree, use it
...
solver_tree = DecisionTreeClassifier(
    random_state=42, max_depth=15, max_leaf_nodes=36, min_samples_leaf=2)
...
print(solver_tree.score(X_tree_tt, y_tree_tt))
...
solver_tree = DecisionTreeClassifier(
    max_depth=15, max_leaf_nodes=36, min_samples_leaf=1)
...
# pruning
print('Searching for best pre-pruning condition(s)...')
ada_pre_pruning = np.zeros((5, 20, 6), dtype=np.float32)
m, n, s = (0, 0, 0)
for i, m in enumerate(np.linspace(1, 15, 5, dtype=np.int16)):
    for j, n in enumerate(np.linspace(2, 166, 20, dtype=np.int16)):
        for k, s in enumerate(np.linspace(1, 12, 6, dtype=np.int16)):
...
solver_tree = DecisionTreeClassifier(
    max_depth=11, max_leaf_nodes=122, min_samples_leaf=2)
solver_ada = AdaBoostClassifier(solver_tree)
...
solver_svm_linear = SVC(kernel='linear', max_iter=-1)
...
print('KNN')
solver_knn6 = KNeighborsClassifier(n_neighbors=3)
t0 = time.time()
solver_nn.fit(X, y)
t1 = time.time()
...
print('SVM linear')
solver_svm_linear = SVC(C=1, kernel='linear', max_iter=-1)
t0 = time.time()
...
solver_svm_poly = SVC(C=1, kernel='poly',
